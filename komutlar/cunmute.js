...
module.exports.run = async (bot, message, args) => {
    ...
    if ( !message.member.roles.find("792372264376074250") && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Yapmak İçin Mute Hammer Yetkisine Sahip Olmalısın.") // KUILLANICAK ROL İDSİ
    ...
    let muterole = message.guild.roles.find(`name`, "Muted"); //MUTELENDİĞİNDE VERİLDİĞİ PERMİN ADINI YAZINIZ..
    ...
    const muteembed = new Discord.RichEmbed()
            .setAuthor('Mutesi Kaldırıldı!')
            .addField('Kullanıcı', `<@${user.id}>`)
            .addField('Sebep', `${reason}`)
            .addField('Mutesini Kaldıran Yetkili:', `${mod}`)
            .setColor('RANDOM')
